...
* Para que o usuário possa utilizar algum serviço do domínio, o processo de autorização se dá através do envio do TGT ao Domain Controller para a requisição do Service Ticket ou TGS (Ticket-Granting-Service), ex.: MSSqlSvc/SQL.domain.com.
...
É necessário ao atacante listar todos os SPN's presentes no domínio. Uma das formas de enumará-los é utilizando o módulo Get-NetUser do [PowerView](https://github.com/PowerShellMafia/PowerSploit/blob/master/Recon/PowerView.ps1) projeto que reúne diversas formas de enumeração em ambiente Microsoft Active Directory, ou poderíamos utilizar a ferramenta [setspn.exe](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-r2-and-2012/cc731241(v%3Dws.11)) seguindo do argumento "-L ServerName", presente por padrão nos sistemas operacionais Microsoft Windows.
